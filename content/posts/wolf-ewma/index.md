...
tldr: "An EWMA is an special case of the KF. The WoLF method is a robust variant of the KF. Thus, we create a 1D version of WoLF that resembles the EWMA and is robust to outliers."
---
...
we apply the WoLF algorithm to a unidimensional state-space model (SSM) with unit state and measurement coefficients.
...
Then, we introduce the unidimensional SSM with unit state and transition coefficients
...
The signal $z_t$ evolves over time according to a random walk with noise $w_t$
and the observations $y_t$ are generated by the signal $z_t$ plus noise $e_t$.

Our aim is to estimate the signal expected $z_t$ given the observations $y_{1:t}$.
{{< math >}}
$$
...
More generally, we estimate the *posterior* distribution $p(z_t \vert y_{1:t})$,
...
and $w_t: \mathbb{R} \to \mathbb{R}$ is a weight function.
...
We observe that the rate $k_t$ is fairly stable, however, it decreases when the measurement $y_t$ is is unusually large.


# Conclusion
In this post, we showed that the EWMA is a special case of the Kalman filter in a one-dimensional state-space model
with unit state and measurement coefficients.
