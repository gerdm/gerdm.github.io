...
Put simply, the SSM model $(2)$ assumes that the signal $z_t$ evolves over time according to a random walk with noise $w_t$,
and the observations $y_t$ are generated by the signal $z_t$ plus noise $e_t$.

Our aim is to estimate the expected signal $z_t$ given the observations $y_{1:t}$.
{{< math >}}
$$
