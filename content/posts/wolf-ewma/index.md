...
Our estimate of the signal $z_t$ given the observations $y_{1:t}$ is
{{< math >}}
$$
...
From $(6)$, we see that the KF applied to the SSM in $(2)$ is equivalent to the EWMA with $\beta$ replaced by $k_t$,
...
Suppose that we want to estimate the EWMA of the DJI on-line, so that we can make trading decisions.
In this experiment, we corrupt the DJI signal with outliers, representing corrupted data.
